...
```bash
sam-rest$ sam local invoke putItemFunction --event events/event-post-item.json
sam-rest$ sam local invoke getAllItemsFunction --event events/event-get-all-items.json
```

The AWS SAM CLI can also emulate your application's API. Use the `sam local start-api` command to run the API locally on port 3000.

```bash
sam-rest$ sam local start-api
sam-rest$ curl http://localhost:3000/
```
...
```bash
sam-rest$ sam logs -n putItemFunction --stack-name sam-app --tail
```
...
```bash
sam-rest$ npm install
# Unit test
sam-rest$ npm run test
# Integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
sam-rest$ AWS_SAM_STACK_NAME=<stack-name> npm run integ-test
```
